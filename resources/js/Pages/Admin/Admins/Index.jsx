...
import { LuPlus, LuPencil, LuTrash2, LuSearch } from "react-icons/lu"; // Tambah LuMoreHorizontal jika ingin menu dropdown nanti
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Kelola Admin</h1>
                    <p className="text-slate-500 mt-1 font-medium text-sm">Atur akses dan peran administrator sistem.</p>
                </div>
                <Link 
                ...
                <div className="p-6 sm:p-8 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-400">
                        <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                            Total: {admins.data.length} Admin
                        </span>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-100">
                                <th className="px-8 py-5 text-xs uppercase font-extrabold text-slate-400 tracking-wider">User Profile</th>
                                <th className="px-8 py-5 text-xs uppercase font-extrabold text-slate-400 tracking-wider">Kontak</th>
                                <th className="px-8 py-5 text-xs uppercase font-extrabold text-slate-400 tracking-wider">Status</th>
                                <th className="px-8 py-5 text-xs uppercase font-extrabold text-slate-400 tracking-wider text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {admins.data.map((admin) => (
                                <tr key={admin.id} className="group hover:bg-slate-50/80 transition-colors duration-200">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            <div className="relative">
                                                <UserAvatar user={admin} className="w-12 h-12 rounded-2xl shadow-sm border-2 border-white ring-1 ring-slate-100" />
                                                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-white rounded-full"></div>
                                            </div>
                                            <div>
                                                <div className="font-bold text-slate-800 text-base">{admin.name}</div>
                                                <div className="text-xs text-slate-400 font-medium mt-0.5">Admin</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                                    ...
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs text-slate-400 font-medium">
                                                • {new Date(admin.created_at).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                    ...
                    {admins.data.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p className="text-slate-500 text-sm max-w-xs mx-auto mt-1">
                                Coba ubah kata kunci pencarian atau tambahkan admin baru.
                            </p>
                        </div>
                    )}
                </div>
